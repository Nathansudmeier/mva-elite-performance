...
export default function Dashboard() {
  ...
  const wins = playedMatches.filter((m) => Number(m.score_home) > Number(m.score_away)).length;
  const losses = playedMatches.filter((m) => Number(m.score_home) < Number(m.score_away)).length;
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" }} className="mobile-grid-1col">
        ...
        <div style={{ background: "#ffffff", border: "2.5px solid #1a1a1a", borderRadius: "18px", boxShadow: "3px 3px 0 #1a1a1a", padding: "1rem" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "12px" }}>
            ...
            <button onClick={() => navigate("/WedstrijdReflecties")} style={{ fontSize: "12px", color: "#FF6800", fontWeight: 700, background: "none", border: "none", cursor: "pointer" }}>Alle →</button>
          </div>
